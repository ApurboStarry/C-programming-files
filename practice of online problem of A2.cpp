...
int* dijkstra(LinkedListDoubly ll[], int V, int src) {
    int *dist = new int[V];
    int prev[V];
    bool sptSet[V];
...
void a2OnlineProblem(LinkedListDoubly ll[], int V, int src, int dest, int u[], int v[], int wt[], int n) {
    int *srcDijkstra = dijkstra(ll, V, src);
    int *destDijkstra = dijkstra(ll, V, dest);
    ...
    */
    ...
    int result = srcDijkstra[dest];
    for(int i = 0; i < n; i++) {
        int first = u[i];
        int second = v[i];
        int cost = wt[i];
        int intermediate = minimum(srcDijkstra[first] + cost + destDijkstra[second], srcDijkstra[second] + cost + destDijkstra[first]);
        //cout << "$$$ Intermediate " << i << " " << intermediate << endl;
        //cout << "$$$ Intermediate " << i << " " << first << " " << second << " " << cost << endl;
        if(intermediate < result)
            result = intermediate;
    }

    if(result != srcDijkstra[dest]) {
        cout << "Changed! And the modified distance is " << result << endl;
    }
    else
        cout << "Unchanged! Actual distance between source and destination is " << result << endl;
}

void b2OnlineProblem() {
...
int main() {
    ...
    cin >> numberOfNewEdges;

    int p[numberOfNewEdges], q[numberOfNewEdges], weight[numberOfNewEdges];

    for(int i = 0; i < numberOfNewEdges; i++) {
        cin >> p[i] >> q[i] >> weight[i];
    }

    a2OnlineProblem(graph, V, src, dest, p, q, weight, numberOfNewEdges);

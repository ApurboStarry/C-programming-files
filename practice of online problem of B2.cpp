...
void dijkstra(LinkedListDoubly ll[], int V, int src, int weightsOfVertices[]) {
    int *dist = new int[V];
    int prev[V];
    bool sptSet[V];
    ...
    dist[src] = weightsOfVertices[src];
    MinHEAP h(V);
    ...
    while(!h.isEmpty()) {
        ...
        while(temp != NULL) {
            int v = temp->item;
            int wt = temp->weight;
            if(sptSet[v] == false && dist[u] != INFINITE && dist[v] > dist[u] + wt + weightsOfVertices[v]) {
                dist[v] = dist[u] + wt + weightsOfVertices[v];
                prev[v] = u;
...
int main() {
    int n, m, u, v, wt;
    cin >> n >> m;
    int weightsOfVertices[n];
    for(int i = 0; i < n; i++) {
        cin >> weightsOfVertices[i];
    }
    ...
    dijkstra(graph, V, src, weightsOfVertices);
